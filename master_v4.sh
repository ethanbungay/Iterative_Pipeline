...
wait_for_lines() {
    ...
    #desired_lines=8  # Replace with the desired number of lines
...
do
    ...
    cp run_mpnn_specify_designed-symmetric.sh "cycle${i}/ProteinMPNN"
    cp esmfold_batch_scores_plots_ext.py "cycle${i}/ESMfold"
    cp esmfold_example_submission.sh "cycle${i}/ESMfold"
...
do
    ...
    cd ProteinMPNN
    sbatch run_mpnn_specify_designed-symmetric.sh        # output = a big fasta file
    ...
    cd ../ESMfold
    sbatch esmfold_example_submission.sh         # output = lots of PDBs, JSONs, pTM_pLDDT_report.csv
